...
var CssPackage, Dependency, JsPackage, coffee, compilers, detective, dirname, fs, resolve, stitch, toArray,
...
CssPackage = (function() {
  ...
  function CssPackage(cssPath) {
    try {
      this.path = require.resolve(resolve(cssPath));
    } catch (err) {
...
JsPackage = (function() {
  ...
  function JsPackage(config) {
    ...
    if (config.main) {
      this.paths.concat([dirname(resolve(config.main))]);
    }
  }
  ...
  JsPackage.prototype.compileModules = function() {
    ...
    this.stitch = new stitch.Stitch(this.paths);
    jsFiles = (function() {
