...
program.command('new [name]').description('  create new project').usage('[name] [options]').option('-t, --template [name]', 'template to use').option('-c, --config [config.json]', 'configuration file to supply context variables from').option('--no-install', 'do not run npm install automatically').action(function(name, opts) {
  if (opts == null) {
    opts = {};
  }
  if (!name) {
